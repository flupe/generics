...
mutual
  ...
  Extend (π e S C) ℓ₂ X pvi@(p , v , i) = Extendb ℓ₂ e X S C pvi
...
module _ {P} {I : ExTele P} where
  ...
  mutual
    ...
    map⟦⟧ ℓ₂ ℓ₃ f (π p S C) = map⟦⟧′ ℓ₂ ℓ₃ f p S C
  ...
  mutual
    ...
    mapExtend ℓ₂ ℓ₃ f (π p S C) x = mapExtend′ ℓ₂ ℓ₃ f p S C x
  ...
  module _ (funext : ∀ {a b} {A : Set a} {B : A → Set b} {f g : (x : A) → B x}
    ...
    mutual
      ...
      map⟦⟧-compose {C = π p S C} x = map⟦⟧′-compose x
      ...
      map⟦⟧-id f≗id (π p S C) x = map⟦⟧′-id f≗id p C x
    ...
    mutual
      ...
      mapExtend-compose {C = π p S C} x = mapExtend′-compose x
      ...
      mapExtend-id f≗id (π p S C) x = mapExtend′-id f≗id p C x
