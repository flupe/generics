...
record HasDesc {P} {I : ExTele P} {ℓ} (A : Curried′ P I ℓ) : Setω where
  ...
  split∘constr funext x = begin
    split (constr x)                                   ≡˘⟨ cong (split ∘ constr)
                                                                (mapD-id funext from∘to {D = D} x) ⟩
    split (constr (mapD ℓ ℓ (from ∘ to) D x))          ≡⟨ cong (split ∘ constr)
                                                               (mapD-compose funext {f = to}
                                                                                    {g = from}
                                                                                    {D = D} x) ⟩
    split (constr (mapD _ ℓ from D (mapD ℓ _ to D x))) ≡⟨ cong split (constr-coh _) ⟩
    split (from ⟨ mapD ℓ _ to D x ⟩)                   ≡⟨ split-coh _ ⟩
    mapD _ _ from D (mapD _ _ to D x)                  ≡˘⟨ mapD-compose funext {f = to} {g = from} {D = D} x ⟩
    mapD _ _ (from ∘ to) D x                           ≡⟨ mapD-id funext from∘to {D = D} x ⟩
    x                                                  ∎
